...
import os
import sys
from YUV import YUV_Snapshot as snap

class Camera:
	def __init__(self, asp1, asp2, cam, name):
		self.width = int(asp1)
		...
		self.snap = snap(self.width, self.height, self.name, self.cam)
		...
		if self.cam not in self.set:
			...
			if idx == 4:
				...
				for i in range(4):
					...
					i2c = "i2cset -y 1 0x70 0x00 {}".format(self.i2c[i][0])
					os.system(i2c)
					...
					#os.system(cmd)
					'''Call class to take YUV picture'''
					self.snap.set_dir(self.set[i])
					self.snap.snap()
			else:
				#i2c = f"i2cset -y 1 0x70 0x00 {self.i2c[idx][0]}"
				i2c = "i2cset -y 1 0x70 0x00 {}".format(self.i2c[idx][0])
				os.system(i2c)
